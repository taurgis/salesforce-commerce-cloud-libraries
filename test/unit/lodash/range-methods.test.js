...
var rangeRight = require('../../../cartridges/lodash/rangeRight');
var each = require('../../../cartridges/lodash/each');
var map = require('../../../cartridges/lodash/map');
var { falsey } = require('../helpers/stubs');

describe('range methods', function () {
    each(['range', 'rangeRight'], function (methodName) {
        var func = (methodName === 'range') ? range : rangeRight;
        var isRange = methodName === 'range';

        function resolve(rangeFn) {
            return isRange ? rangeFn : rangeFn.reverse();
        }
        ...
        it('`_.' + methodName + '` should treat falsey `start` as `0`', function () {
            each(falsey, function (value, index) {
                if (index) {
        ...
        it('`_.' + methodName + '` should work as an iteratee for methods like `_.map`', function () {
            ...
            var expected = map([[0], [0, 1], [0, 1, 2]], resolve);

            each([array, object], function (collection) {
                var actual = map(collection, func);
