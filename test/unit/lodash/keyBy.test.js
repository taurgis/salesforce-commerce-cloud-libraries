...
var keyBy = require('../../../cartridges/lodash/keyBy');
...
describe('keyBy', function () {
    ...
    it('should use `_.identity` when `iteratee` is nullish', function () {
        ...
        var expected = map(values, constant({ '4': 4, '6': 6 }));
    ...
    it('should work in a lazy sequence', function () {
        ...
        var actual = _(array).keyBy().map(square).filter(isEven)
            .take()
            .value();

        assert.deepEqual(actual, take(filter(map(keyBy(array), square), isEven)));
    });
});
