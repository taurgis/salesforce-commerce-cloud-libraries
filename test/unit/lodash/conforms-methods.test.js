...
var conformsTo = require('../../../cartridges/lodash/conformsTo');
...
describe('conforms methods', function () {
    each(['conforms', 'conformsTo'], function (methodName) {
        var isConforms = methodName === 'conforms';

        function conformsFunction(source) {
            return isConforms ? conforms(source) : function (object) {
        ...
        it('`_.' + methodName + '` should not match by inherited `source` properties', function () {
            ...
            var par = conformsFunction(new Foo());
            var actual = filter(objects, par);
        ...
        it('`_.' + methodName + '` should work with a function for `source`', function () {
            ...
            var objects = [{ 'a': 1 }, { 'a': 2 }];
            var actual = filter(objects, conforms(Foo));
