...
var truncate = require('../../../cartridges/lodash/truncate');
...
describe('truncate', function () {
    ...
    it('should coerce `length` to an integer', function () {
        each(['', NaN, 4.6, '4'], function (length, index) {
            var actual = index > 1 ? 'h...' : '...';
            assert.strictEqual(truncate(string, { 'length': { 'valueOf': constant(length) } }), actual);
        });
    });
    ...
    it('should work as an iteratee for methods like `_.map`', function () {
        var actual = map([string, string, string], truncate);
        var truncated = 'hi-diddly-ho there, neighbo...';
